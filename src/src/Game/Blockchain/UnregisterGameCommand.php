...
#[AsCommand(
    name: 'game:blockchain:unregister',
    description: 'Unregister a game from the Monad blockchain',
)]
final class UnregisterGameCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ->addArgument(
                ...
                'The game contract address to unregister',
            )
            ->addOption(
                'dry-run',
                'd',
            ...
            ->setHelp(
                <<<'EOF'
                    The <info>%command.name%</info> command unregisters a game from the blockchain:

                      <info>php %command.full_name% 0x1234...abcd</info>

                    Skip confirmation:
                      <info>php %command.full_name% 0x1234...abcd --force</info>
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!preg_match('/^0x[a-fA-F0-9]{40}$/', (string) $gameAddress)) {
        ...
        $io->title('Unregister Game from Blockchain');
        $io->table(
            ['Parameter', 'Value'],
            [
                ['RPC URL', $this->rpcUrl],
        ...
        if (!$force && !$isDryRun) {
            if (!$io->confirm("Are you sure you want to unregister game at address '{$gameAddress}' from the blockchain?", false)) {
        ...
        try {
            ...
            $balance = null;
            $web3->eth->getBalance($serverAddress, static function ($err, $result) use (&$balance): void {
                if ($err !== null) {
            ...
            $functionCall = $contract->getData('unregisterGame', $gameAddress);
            ...
            $nonce = null;
            $web3->eth->getTransactionCount($serverAddress, 'pending', static function ($err, $result) use (&$nonce): void {
                if ($err !== null) {
            ...
            // Get gas price
            $gasPrice = null;
            $web3->eth->gasPrice(static function ($err, $result) use (&$gasPrice): void {
                if ($err !== null) {
            ...
            // Send transaction
            $txHash = null;
            $web3->eth->sendRawTransaction('0x' . $signedTx, static function ($err, $result) use (&$txHash): void {
                if ($err !== null) {
            ...
            $io->success([
                'Game unregistered successfully!',
            ...
            while (!$confirmed && $attempts < 30) {
                ...
                $receipt = null;
                $web3->eth->getTransactionReceipt($txHash, static function ($err, $result) use (&$receipt): void {
                ...
                if ($receipt) {
                    ...
                    if ($receipt->status === '0x1') {
                        $io->success([
                            ...
                            '',
                            "Game at address '{$gameAddress}' successfully unregistered from blockchain",
                        ]);
                    } else {
            ...
            $io->error([
                'Failed to unregister game from blockchain',
                $e->getMessage(),
            ]);

            $this->logger->error('Blockchain game unregistration failed', [
